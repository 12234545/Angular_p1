...
export class ChatbotService {
  ...
  private apiUrl = 'https://92f4f66bd25a.ngrok-free.app/api/chat';
  private baseUrl = 'https://92f4f66bd25a.ngrok-free.app';
  ...
  sendMessage(message: string, format: string = 'default'): Observable<any> {
    ...
    return this.http.post<any>(this.apiUrl, requestBody, {
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status === 0) {
          errorMessage = 'Impossible de se connecter au serveur. Vérifiez que l\'URL ngrok est correcte et que Colab fonctionne.';
        } else if (error.status === 404) {
          errorMessage = 'Endpoint non trouvé. Vérifiez l\'URL de l\'API.';
        } else if (error.status === 422) {
          errorMessage = 'Erreur de validation des données. Vérifiez le format du message.';
  ...
  testConnection(): Observable<any> {
    ...
    return this.http.get<any>(testUrl, { headers }).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status === 0) {
          errorDetails = 'Serveur non accessible. Vérifiez l\'URL ngrok et que Colab fonctionne.';
        }
  ...
  isConfigured(): boolean {
    return this.apiUrl !== 'https://92f4f66bd25a.ngrok-free.app/api/chat';
  }
